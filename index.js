...
`).join("");l.innerHTML=e,L.refresh()}function b(){l.innerHTML=`
  ...
  `}document.querySelector(".gallery");const w=document.querySelector(".search-form");new d(".gallery a",{captionsData:"alt",captionDelay:250});function c(){document.getElementById("loading-overlay").style.display="flex"}function p(){document.getElementById("loading-overlay").style.display="none"}const E=["nature","technology","art","food","travel","sports","animals","architecture","people","music"];function P(r,e){return r.slice().sort(()=>.5-Math.random()).slice(0,e)}async function S(){const r=Math.floor(Math.random()*4)+2,e=P(E,r).join(",");try{c();const s=await u(e);m(s)}catch{i.error({title:"Error",message:"Failed to load images. Please try again.",position:"topRight"})}finally{p()}}S();w.addEventListener("submit",async r=>{c(),r.preventDefault();const e=r.target.elements.searchQuery.value.trim();if(!e){i.warning({title:"Error",message:"Please enter a search term!",position:"topRight"});return}c();try{const s=await u(e);m(s)}catch{i.error({title:"Error",message:"Failed to load images. Please try again.",position:"topRight"})}finally{p()}});
